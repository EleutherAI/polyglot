...
from transformers import ForwardContext, ModelWithFlexibleHeadsAdaptersMixin
...
try:
    ...
    raise ImportError(
        "If you want to train MAD-X model, "
        "you must install adapter-transformers rather than transformers. "
        "Please install it by `pip install adapter-transformers`."
    )


class GPTNeoDecoderBlockAdaptersMixin:
...
class GPTNeoModelAdapterMixin(InvertibleAdaptersMixin, ModelAdaptersMixin):
...
class GPTNeoModel(GPTNeoModelAdapterMixin, GPTNeoPreTrainedModel):
    ...
    def forward(
        ...
        inputs_embeds = self.invertible_adapters_forward(inputs_embeds, rev=False)
        ...
        hidden_states = self.invertible_adapters_forward(hidden_states, rev=True)
...
class GPTNeoForCausalLM(ModelWithHeadsAdaptersMixin, GPTNeoPreTrainedModel):
...
class GPTNeoForSequenceClassification(
    ModelWithHeadsAdaptersMixin, GPTNeoPreTrainedModel
):
...
@add_start_docstrings(
    """
The GPTNeo Model that allows the loading of different heads dor different tasks. This enables a flexible use of the
...
class GPTNeoAdapterModel(ModelWithFlexibleHeadsAdaptersMixin, GPTNeoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.transformer = GPTNeoModel(config)
...
class GPTNeoModelWithHeads(GPTNeoAdapterModel):
